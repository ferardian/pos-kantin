...
const isFewSearchResults = computed(() => {
    const q = searchQuery.value?.trim();
    return Boolean(q && q.length > 0 && filteredProducts.value.length > 0 && filteredProducts.value.length <= 4);
});
    ...
    <MainLayout>
        <div class="flex flex-col lg:flex-row h-full w-full min-h-0 bg-slate-100 overflow-hidden font-sans relative">
            ...
            >
                ...
                <div v-if="searchQuery" class="px-4 py-2 bg-amber-50/90 border-b border-amber-200/80 flex items-center justify-between text-xs text-amber-950 font-medium shrink-0">
                    <div class="flex items-center gap-2">
                        <Search class="w-3.5 h-3.5 text-amber-600 shrink-0" />
                        <span>Ditemukan <strong>{{ filteredProducts.length }}</strong> menu untuk "<strong>{{ searchQuery }}</strong>"</span>
                        <span v-if="isFewSearchResults" class="text-[10px] bg-amber-200/80 text-amber-900 px-2 py-0.5 rounded-md font-extrabold uppercase tracking-wide">
                            Mode Sentuh Besar
                        </span>
                    </div>
                    <button 
                ...
                <!-- Product Grid List (Responsive & Dynamic Adaptive) -->
                <div 
                    class="flex-1 min-h-0 overflow-y-auto overscroll-y-contain p-3 sm:p-4 pb-28 lg:pb-6 content-start items-start transition-all duration-150"
                    :class="[
                        isFewSearchResults 
                ...
                >
                    <div 
                        ...
                        :class="[
                            getItemQtyInCart(product.id) > 0 
                                ? 'border-amber-400 bg-amber-50/20 ring-2 ring-amber-400/80 shadow-md' 
                                : 'border-slate-200/90 bg-white hover:border-amber-400 hover:shadow-md',
                            isFewSearchResults ? 'p-4 sm:p-5 rounded-2xl' : 'p-3 sm:p-3.5 rounded-2xl',
                            product.units.length === 1 ? 'cursor-pointer active:scale-[0.99]' : ''
                        ]"
                        class="flex flex-col justify-between transition-all duration-150 group shadow-xs border relative select-none"
                    >
                        <!-- In-Cart Top Floating Badge -->
                        <div 
                            v-if="getItemQtyInCart(product.id) > 0" 
                            :class="isFewSearchResults ? 'text-xs px-2.5 py-1 -top-2.5 -right-2' : 'text-[10px] px-2 py-0.5 -top-2 -right-2'"
                            class="absolute bg-amber-500 text-slate-950 font-black rounded-full shadow-md flex items-center gap-1 border-2 border-white z-10 animate-in fade-in zoom-in duration-150"
                        >
                        ...
                        <div>
                            ...
                            <div class="flex items-center justify-between text-[11px] mb-1.5">
                                <span :class="isFewSearchResults ? 'text-xs' : 'text-[10px]'" class="text-slate-500 font-bold uppercase truncate">
                                    {{ product.brand?.name || 'Kantin' }}
                                </span>
                                <span 
                                    :class="[
                            ...
                            <h3 
                                :class="[
                                    getItemQtyInCart(product.id) > 0 ? 'text-amber-950 font-black' : 'text-slate-900 font-bold',
                                    isFewSearchResults ? 'text-sm sm:text-base mb-2' : 'text-xs mb-1.5'
                                ]" 
                                class="group-hover:text-amber-600 transition line-clamp-2 leading-snug"
                            >
                                {{ product.name }}
                            </h3>

                            <!-- SKU -->
                            <p :class="isFewSearchResults ? 'text-xs mb-3' : 'text-[10px] mb-2'" class="font-mono text-slate-400">
                                {{ product.sku }}
                            </p>
                        </div>
                        ...
                        <div class="space-y-2 pt-2.5 border-t border-slate-100">
                            <template v-for="unit in product.units" :key="unit.id">
                                ...
                                <div 
                                    ...
                                    :class="[
                                        isFewSearchResults 
                                            ? 'px-4 py-3 rounded-2xl min-h-[56px] bg-amber-50 hover:bg-amber-500 text-amber-950 hover:text-slate-950 border-2 border-amber-300 hover:border-amber-500 shadow-xs' 
                                            : 'px-3 py-2 rounded-xl min-h-[44px] bg-slate-50 hover:bg-amber-500 text-slate-800 hover:text-slate-950 border border-slate-200/90 hover:border-amber-500 shadow-2xs',
                                        'flex items-center justify-between cursor-pointer transition-all duration-150 active:scale-[0.98] group/unit'
                                    ]"
